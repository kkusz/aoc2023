         ...
         01 WS-CARD-STATS.
            ...
            05 WS-BEST-CARD        PIC 99.
            05 WS-BEST-COUNT       PIC 9.
         ...
         CONVERT-CARD SECTION.
            ...
            PERFORM VARYING IX FROM 1 BY 1 UNTIL IX > 5
               ...
               IF IX2 > 1
               AND WS-CARD-NO(IX2) > WS-BEST-COUNT
                  MOVE IX2 TO WS-BEST-CARD
                  MOVE WS-CARD-NO(IX2) TO WS-BEST-COUNT
               END-IF
            END-PERFORM
         ...
         DETERMINE-TYPE SECTION.
              IF WS-CARD-NO(1) = 5
                 ADD 1 TO WS-STAT-FIVES
              ELSE
                 ADD WS-CARD-NO(1) TO WS-CARD-NO(WS-BEST-CARD)
                 PERFORM VARYING IX2 FROM 2 BY 1 UNTIL IX2 > 14
                    EVALUATE WS-CARD-NO(IX2)
                       WHEN 5 
                          ADD 1 TO WS-STAT-FIVES
                       WHEN 4
                          ADD 1 TO WS-STAT-FOURS
                       WHEN 3 
                          ADD 1 TO WS-STAT-TRIPLES
                       WHEN 2
                          ADD 1 TO WS-STAT-PAIRS
                    END-EVALUATE
                 END-PERFORM
              END-IF
